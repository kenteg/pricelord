...
class PriceLordBot(
    ...
    @PostConstruct
    fun addHandlerExecuteMessageFunction() {
        messageHandler.executeMessage = { message ->  this.execute(message) }
    }
    ...
    private fun handleMessage(update: Update) {
        ...
        if (message == "/start") {
            messageHandler.handleStart(update)
        } else {
    ...
    private fun handleCallback(update: Update) {
        ...
        when(data) {
            ADD_ITEM.name -> messageHandler.handleAddItem(update)
        }
        if (ADD_ITEM.name == data) {
